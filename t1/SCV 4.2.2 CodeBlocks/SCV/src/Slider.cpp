...
void Slider::createTexture(void) {
   ...
   if ((_cTexture = kernel->getWidgetTexture(SLIDER)) != NULL) return;

   // create texture object
   _cTexture = new ComponentTexture(16, 32);
   kernel->setWidgetTexture(SLIDER, _cTexture);
...
void Slider::processMouse(const scv::MouseEvent &evt) {
   ...
   if (isFocused()) {
      if (isInside(evt.getPosition()) && evt.getState() == MouseEvent::CLICK) {
         _isDragging = false;
         double value = (static_cast<double>(evt.getPosition().x-(getAbsolutePosition().x+4))/(getWidth()-8)) * (static_cast<double>(getMaxValue()-getMinValue())) + static_cast<double>(getMinValue());
