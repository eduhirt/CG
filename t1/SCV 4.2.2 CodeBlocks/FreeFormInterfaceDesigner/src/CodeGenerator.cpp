...
std::string CodeGenerator::addChildren(scv::Component *child, std::string dad, bool customClass) {
   ...
   if(type == scv::Component::PANEL) { // Panel
      ...
      y = child->getRelativePosition().y + child->getHeight();
      if(customClass == false)
         code += "   scv::ComboBox *comboBox" + scv::toString(count[child->getType()]) + " = new scv::ComboBox(scv::Point(" + scv::toString(child->getRelativePosition()) + "), " + scv::toString(child->getWidth()) + ");\n";
      else
         code += "   ComboBox" + scv::toString(count[child->getType()]) + " *comboBox" + scv::toString(count[child->getType()]) + " = new ComboBox" + scv::toString(count[child->getType()]) + "(scv::Point(" + scv::toString(child->getRelativePosition()) + "), " + scv::toString(child->getWidth()) + ");\n";
...
std::string CodeGenerator::createAllocationCode(scv::Component *comp, bool customClass) {
   ...
   if(type == scv::Component::PANEL) { // Panel
      ...
      y = comp->getRelativePosition().y + comp->getHeight();
      if(customClass == false)
         code += "   scv::ComboBox *comboBox" + scv::toString(count[comp->getType()]) + " = new scv::ComboBox(scv::Point(" + scv::toString(comp->getRelativePosition()) + "), " + scv::toString(comp->getWidth()) + ");\n";
      else
         code += "   ComboBox" + scv::toString(count[comp->getType()]) + " *comboBox" + scv::toString(count[comp->getType()]) + " = new ComboBox" + scv::toString(count[comp->getType()]) + "(scv::Point(" + scv::toString(comp->getRelativePosition()) + "), " + scv::toString(comp->getWidth()) + ");\n";
...
std::string CodeGenerator::createCustomClassCodeH(scv::Component *comp) {
    ...
    if(type == scv::Component::PANEL) { // Panel
      ...
      code += "public:\n";
      code += "   ComboBox" + scv::toString(count[comp->getType()]) + "(scv::Point p1, int width);\n";
      count[comp->getType()] += 1;
...
std::string CodeGenerator::createCustomClassCodeCpp(scv::Component *comp) {
   std::string code, childCode, ComponentName;
    ...
    if(type == scv::Component::PANEL) { // Panel
      ...
      code += "\n//" + ComponentName;
      code += "\n" + ComponentName + "::" + ComponentName + "(scv::Point p1, int width) : scv::ComboBox(p1, width) {\n}";
      count[comp->getType()] += 1;
   } else if(type == scv::Component::CANVAS) {
...
void CodeGenerator::generateCode(bool customClass) {
   ...
   std::string applicationDotCpp =
      ...
      "\n"
      "   setWindowTitle(\"SCV v4.2.2 - FreeForm Interface Designer\");\n"
      "}\n"
      "\n"
   ...
   outputFile.open("generatedCode/Application.h");
   outputFile << applicationDotH;
   outputFile.close();

   outputFile.open("generatedCode/Application.cpp");
   outputFile << applicationDotCpp;
   outputFile.close();

   outputFile.open("generatedCode/main.cpp");
   outputFile << mainDotCpp;
   outputFile.close();

   outputFile.open("generatedCode/Widget.h");
   outputFile << widgetDotH;
   outputFile.close();

   outputFile.open("generatedCode/Widget.cpp");
   outputFile << widgetDotCpp;
   outputFile.close();

   std::cout << "Files generated at folder generatedCode" << std::endl;
}
...
std::string CodeGenerator::generateCodeAppCpp(void)
{
   ...
   applicationDotCpp += "\n";
   applicationDotCpp += "   setWindowTitle(\"SCV v4.2.2 - FreeForm Interface Designer\");\n";
   applicationDotCpp += "}\n";
   applicationDotCpp += "\n";
