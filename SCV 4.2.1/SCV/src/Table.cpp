...
#include "SCV/util.h"
#include "SCV/GroupLayout.h"

namespace scv {

Table::Table( const scv::Point &p, int nRows /*= 4*/, int nColumns /*= 4*/, int nCellLines /*= 1*/, int CellWidth /*= 1*/ ) :
   Panel(p, Point(p.x + nColumns * (CellWidth - 1) + 10/*1*/, 20 + p.y + (TextBox::s_lineSpacing * nCellLines + TextBox::s_borderHeight) * nRows + 20/*1*/)), _nRows(nRows), _nColumns(nColumns), _nCellLines(nCellLines), _cellWidth(CellWidth) {
...
bool Table::alphabetical(std::string str1, std::string str2) {
   int i = 0;
   while (i < (int)str1.length() && i < (int)str2.length() && i >= 0) {
...
void Table::align(int column) {
   if (column >= 0 && column < _nColumns) {
      ...
      for (int t = 0 ; t < (int)_table.size() ; t++ ) {
         max = 0;
         _max = 0;
         index = -1;
         ...
         for (int j = 0; j < (int)_table[index].size(); j++) {
