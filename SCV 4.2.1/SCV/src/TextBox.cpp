...
void TextBox::display(void) {
    ...
    for (int i = _firstLine; i < (_nLines+_firstLine) && i < ((int)_lineIndex.size() - 1); i++)
...
void TextBox::refreshCursor(void) {
    ...
    if(isFocused()){
        ...
        for (int i = 0 ; i < (int)_lineIndex.size()-1 ; i++) {
...
void TextBox::processMouse(const scv::MouseEvent &evt) {
    ...
    if (!isEditable())
    {
        if (isFocused()) {
            if (evt.getState() == MouseEvent::WHEELDOWN) {
                if ((int)_str.size() > _lineIndex[_firstLine+1] && (int)_lineIndex.size() > _firstLine+3/*De alguma maneira isso faz sentido*/){
                    _firstLine++;
                    return;
                }
            }
    ...
    if (isFocused()) {
        if (evt.getState() == MouseEvent::WHEELDOWN) {
            if ((int)_str.size()>_lineIndex[_firstLine+1]) {
                _firstLine++;
                return;
            }
        }
    ...
    if ((evt.getState() == MouseEvent::CLICK || evt.getState() == MouseEvent::HOLD) && isFocused()) {
        ...
        if (index.y >= (int)_lineIndex.size()-1) {
    ...
    if (evt.getPosition() >= getAbsolutePosition() && evt.getPosition() <= (getSize() + getAbsolutePosition()) && getParentScissor().isInside(evt.getInversePosition())) {
        if (evt.doubleClick()) {
            ...
            if (_currChar==(int)_str.size()-1 || (int)_str.size()==0 || _currChar==-1)
                return;
            ...
            if (_str[_selectStart+1]==' ')
                _selectStart++;
            while (_str[_selectEnd]!=' ' && _selectEnd!=(int)_str.size()-1)
                _selectEnd++;
            if (_str[_selectEnd]==' ')
                _selectEnd--;
...
void TextBox::processKey(const scv::KeyEvent &evt) {
    ...
    if (isFocused()) {
        ...
        case GLUT_KEY_DOWN:
            if (evt.modifiers == GLUT_ACTIVE_SHIFT) {
                if (!(_lineIndex[_desloc.y+1] == (int)_str.size())) {
                    if (!hasSelectedText()) {
                        _selectStart = _currChar;
                        downLine();
                        _selectEnd = _currChar;
...
void TextBox::selectLine(int line) {
    ...
    if ((int)_lineIndex.size() > line && line > 0) {
        kernel->requestComponentFocus(this);
...
void TextBox::setCursorPosition(int position) {
    if (position < (int)getString().size()) {
        _currChar = position;
        refreshText();
    }
}
